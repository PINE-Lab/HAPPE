% PURPOSE  : 	Calculates (normalized) cross covariance between each epoch, at the specified channel(s), with a
%                 simulated blink-like waveform
%
% FORMAT   :
%
% >> EEG = pop_artblink(EEG, parameters);
%
% INPUTS   :
%
% EEG           - input dataset
...
% EXAMPLE  :
%
% >> EEG = pop_artblink(EEG, 'Twindow', [-200 800], 'Blinkwidth', 400, 'Crosscov', 0.6, 'Channel', 1:16, 'Flag', 1);
%
...
function [EEG, com] = pop_artblink(EEG, varargin)
com = '';
if nargin<1
        help pop_artblink
        return
...
if nargin==1
        serror = erplab_eegscanner(EEG, 'pop_artblink', 2, 0, 1, 2, 1);
        if serror
                return
        end
...
if ~isempty(find(chanArray<1 | chanArray>EEG(1).nbchan, 1))
        error('ERPLAB says: error at pop_artblink(). Channel indices cannot be greater than EEG.nbchan')
end
if ~isempty(find(flag<1 | flag>16, 1))
        error('ERPLAB says: error at pop_artblink(). Flag cannot be greater than 16 or lesser than 1')
end
...
%
if length(EEG) > 1
        options1 = {'Twindow', p.Results.Twindow, 'Blinkwidth', p.Results.Blinkwidth, 'Crosscov', p.Results.Crosscov,...
                    'Channel', p.Results.Channel, 'Flag', p.Results.Flag, 'Review', p.Results.Review, 'History', 'gui'};
        [ EEG, com ] = eeg_eval( 'pop_artblink', EEG, 'warning', 'on', 'params', options1);
        return;
end
...
com = sprintf( '%s  = pop_artblink( %s ', inputname(1), inputname(1));
for q=1:length(fn)
        fn2com = fn{q};
