...
for currFile = 1:length(FileNames)
    ...
    try
        if ~reprocess
            ...
            %% DETECT BAD CHANNELS PT.1
            % If bad channel detection is on, first check for and remove
            % flatline channels. Then, if running before wavelet 
            % thresholding, detect and reject bad channels using the
            % happe_detectBadChans function (see function for further
            % documentation). Regardless of if detecting bad channels,
            % updated the number of channels included in processing (good
            % channels), the percent of good channels selected (100% if
            % detection is disabled), and the list of channels, if any,
            % marked as bad.
            origChans = EEG.chanlocs ;
            if params.badChans.rej
                % Detect Flatline Channels
                EEG = pop_clean_rawdata(EEG, 'FlatlineCriterion', 3, ...
    ...
    %                 rethrow(ME) ;
            ...
            if params.badChans.rej && params.badChans.order
                EEG = happe_detectBadChans(EEG, params, 2) ;
                dataQC = happe_badChansQC(EEG, dataQC, chanIDs, currFile) ;
                EEG.setname = [EEG.setname '_badc'] ;
                pop_saveset(EEG, 'filename', strrep(FileNames{currFile}, ...
                    inputExt, 'wavclean_badchanrej.set')) ;
            end
